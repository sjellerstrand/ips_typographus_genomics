...
setwd(paste(OUTDIR, "/", DATA, sep="")) ## The directory where the files are located
chunkfile<-paste(VCF, "_", DATA, "_chunks.out", sep="") ## RADpainter output file
mcmcfile<-paste(VCF, "_", DATA, "_chunks.mcmc.xml", sep="") ## finestructure mcmc file
treefile<-paste(VCF, "_", DATA, "_chunks.mcmcTree.xml", sep="") ## finestructure tree file
### 2) EDIT THIS PATH TO WHERE YOU WANT THE PLOTS:
plotsFolder <- paste(OUTDIR, "/", DATA, sep="")
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- paste(VCF, "_", DATA, sep="");  maxIndv <- 10000; maxPop<-10000


### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source(paste(fineRAD, "/FinestructureLibrary.R", sep=""), chdir = TRUE) # read in the R functions, which also calls the needed packages
...
colnames(dataraw) <- substring(colnames(dataraw), 2, length(colnames(dataraw)))
...
jpeg(file=paste(plotsFolder, "/", analysisName,"_SimpleCoancestry.jpeg",sep=""), width=2000, height=2000, quality=100)
#pdf(file=paste(plotsFolder, "/", analysisName,"_SimpleCoancestry.pdf",sep=""),height=25,width=25)
...
jpeg(file=paste(plotsFolder, "/", analysisName,"_PopAveragedCoancestry.jpeg",sep=""), width=2000, height=2000, quality=100)
#pdf(file=paste(plotsFolder, "/", analysisName,"_PopAveragedCoancestry.pdf",sep=""),height=20,width=20)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=1.1,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1.2))
dev.off()
...
xcrt=0
ycrt=45
jpeg(file=paste(plotsFolder, "/", analysisName,"_PopAveragedCoancestry2.jpeg",sep=""), width=2000, height=2000, quality=100)
#pdf(file=paste(plotsFolder, "/", analysisName,"_PopAveragedCoancestry2.pdf",sep=""),height=25,width=25)
