...
b <- ggplot(pca, aes(PC1, PC2), col)  + geom_point(size = 3, aes(color=Locality)) + coord_equal() + theme_light() + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + scale_color_viridis(discrete = T)
