...
samples <- read.delim(paste(METADATA, "/sample_info_1.txt", sep=""), header=T)
samples[,1] <- as.character(samples[,1])
out <- data.frame(matrix(NA, 320, 2))
for(i in 1:nrow(samples)) {
     if(length(which(ind_depth[,1] == samples[i,1])) > 0) {
         out[i,1:2] <- c(samples[i,1], ind_depth[which(ind_depth[,1] == samples[i,1]), 3])
     } else {
         out[i,1:2] <- c(samples[i,1], NA)
     }
}
write.table(out, paste(VCF_OUT, "_sorted_idepth.txt", sep=""), row.names=F, col.names=F, quote=F)
